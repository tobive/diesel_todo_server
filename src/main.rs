...
#[get("/api/list?<query>")]
fn return_list(query: Option<&RawStr>, state: State<AppState>) -> String {
    ...
    match query {
        None => match get_todo_all(&conn) {
            ...
            Err(_err) => format!("Error getting from db.\n"),
        },
...
fn post_data<'r>(todo: Json<TodoPost>, state: State<'r, AppState>) -> String {
    let conn = state.connection.lock().unwrap();
    match create_todo(&conn, &todo.0.title, &todo.0.content) {
        Ok(_res) => format!("Successfully posted.\n"),
        Err(_err) => format!("Error saving to db.\n"),
    }
}

#[delete("/api/delete?<query>")]
fn delete_item(query: i32, state: State<AppState>) -> String {
    let conn = state.connection.lock().unwrap();
    match delete_todo(&conn, query) {
        Ok(_res) => format!("Success deleting.\n"),
        Err(_err) => format!("Error deleting.\n"),
    }
}
